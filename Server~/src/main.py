...
async def server_lifespan(server: FastMCP) -> AsyncIterator[dict[str, Any]]:
    ...
    # Start the stale-session sweeper (idempotent; harmless in stdio mode
...
@mcp.custom_route("/plugin/diagnostics", methods=["GET"])
async def plugin_diagnostics_route(_: Request) -> JSONResponse:
    data = await PluginHub.get_diagnostics(server_version=get_package_version())
...
def _warn_if_remote_host(host: str | None) -> None:
    """Warn when the unauthenticated plugin hub binds off-loopback.

    Remote binding is an explicit opt-in feature, so this only warns.
    """
    normalized = (host or "").strip().lower()
    if not normalized or normalized in _LOOPBACK_HOSTS:
        return
    logger.warning(
        "MCP for Unity Server 绑定到非 loopback 地址 %s：/hub/plugin WebSocket hub 无认证，"
        "局域网内任意客户端可注册伪造工程会话并接收/执行 Unity 命令，请仅在受信网络使用。",
        host,
    )
