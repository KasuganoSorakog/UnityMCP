    ...
    **kwargs,
    ...
    if _is_http_transport():
        ...
        try:
            ...
            # send_command now absorbs and reclassifies its own timeouts, so
            # this only fires for rare TimeoutErrors from other sources; stay
            # neutral and cautious rather than promising a safe retry.
            return normalize_unity_response(
                MCPResponse(
                    success=False,
                    error=(
                        f"Unity 命令 '{command_type}' 超时；"
                        "若为变更类命令，重试前请先验证场景状态"
                    ),
                    hint="retry",
                    retryable=True,
            ...
            # Fail fast; manual reconnect is handled from the Unity MCP panel.
            return normalize_unity_response(
                MCPResponse(success=False, error=err,
                            hint="manual_connect", retryable=False).model_dump()
            )
