...
class UnityConnection:
    ...
    def send_command(self, command_type: str, params: dict[str, Any] = None) -> dict[str, Any]:
        ...
        # Preflight: if Unity reports reloading, fail once and let the user reconnect from the panel.
        try:
            status = read_status_file(target_hash)
            if status and (status.get('reloading') or status.get('reason') == 'reloading'):
                return MCPResponse(
                    success=False,
                    error="Unity is reloading; reconnect from the MCP panel when ready",
                    hint="manual_connect",
                    retryable=False,
                )
        except Exception as exc:
...
def send_command_with_retry(
    ...
    *,
    ...
    """Send a command to a Unity instance once.

    Args:
        command_type: The command type to send
        params: Command parameters
        instance_id: Optional Unity instance identifier (name, hash, name@hash, etc.)
        max_retries: Ignored; retained for API compatibility.
        retry_ms: Retained for response metadata only.

    Returns:
        Response dictionary or MCPResponse from Unity

    Reload/reconnect loops are intentionally disabled. When Unity is unavailable,
    the user must reconnect from the MCP panel.
    """
