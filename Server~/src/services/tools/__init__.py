...
def choose_unity_instance(ctx: Context, explicit: str | None = None) -> str | None:
    """Use a per-call target when provided, otherwise fall back to session state."""
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()
    return get_unity_instance_from_context(ctx)
