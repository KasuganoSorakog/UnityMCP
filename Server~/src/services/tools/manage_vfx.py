...
1. Read `mcpforunity://scene/gameobject/{instanceID}/components` to check if the component exists
2. Use `manage_components` with `action="add", component_type="ParticleSystem"` (or LineRenderer/TrailRenderer/VisualEffect) to add the component first
...
- particle_set_main: Set main module (duration, looping, start_lifetime, start_speed, start_size, start_color, gravity_modifier, max_particles)
- particle_set_emission: Set emission (rate_over_time, rate_over_distance)
- particle_set_shape: Set shape (shape_type, radius, angle, arc, position, rotation, scale)
...
- **Asset Management:**
  - vfx_create_asset: Create a new VFX Graph asset file (requires: asset_name, optional: folder_path, template, overwrite)
  - vfx_assign_asset: Assign a VFX asset to a VisualEffect component (requires: target, asset_path)
