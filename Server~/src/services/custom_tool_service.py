    ...
    async def _poll_until_complete(
        ...
        while True:
            ...
            try:
                ...
                logger.debug(f"Polling {tool_name} stopped after failure: {exc}")
                return MCPResponse(
                    success=False,
                    error=f"{tool_name} polling failed; reconnect from the MCP panel",
                    hint="manual_connect",
                    retryable=False,
                    data={"reason": "poll_failed", "detail": str(exc)},
                )
