...
STALE_STATUS_THRESHOLD_MS = 10_000
...
def _enrich_advice_and_staleness(state_v2: dict[str, Any]) -> dict[str, Any]:
    ...
    # Unity 在失焦或资源导入时会降低 Editor tick 频率，2 秒阈值会把正常连接误判为不可用。
    is_stale = age_ms > STALE_STATUS_THRESHOLD_MS
    ...
    transport = state_v2.get("transport") or {}
    bridge_connected = (
        transport.get("unity_bridge_connected")
    ...
    # 活跃桥接刚刚成功返回本次请求时，旧快照只影响时间精度，不应阻塞后续工具。
    if is_stale and bridge_connected is not True:
...
async def get_editor_state(ctx: Context) -> MCPResponse:
    ...
    return await get_editor_state_for_instance(ctx, unity_instance)


async def get_editor_state_for_instance(
    ctx: Context,
    unity_instance: str | None = None,
) -> MCPResponse:
    """Return editor readiness for a specific Unity instance."""
    ...
    # 命令已从中心 Server 往返 Unity 成功，此处比 Unity 内部缓存更能代表真实桥接状态。
    transport_section = state_v2.get("transport")
    if not isinstance(transport_section, dict):
        transport_section = {}
        state_v2["transport"] = transport_section
    transport_section["unity_bridge_connected"] = True
    transport_section["last_message_unix_ms"] = _now_unix_ms()
