    ...
    public static class GameObjectSerializer
    {
        ...
        public static object GetComponentData(Component c, bool includeNonPublicSerializedFields = true)
        {
            ...
            foreach (var propInfo in cachedData.SerializableProperties)
            {
                ...
                // ParticleSystem module structs expose internal getters that are not stable serialization contracts.
                if (componentType == typeof(ParticleSystem) &&
                    propInfo.PropertyType.DeclaringType == typeof(ParticleSystem) &&
                    propInfo.PropertyType.Name.EndsWith("Module", StringComparison.Ordinal))
                {
                    skipProperty = true;
                }
                ...
                try
                {
                    ...
                    if (!Application.isPlaying && (propName == "material" || propName == "materials" || propName == "mesh"))
                    {
                        ...
                        else if (propName == "material" && c is Collider collider)
                        {
                            value = collider.sharedMaterial;
                        }
                        else if (propName == "material" && c is Collider2D collider2D)
                        {
                            value = collider2D.sharedMaterial;
                        }
                        else
                        {
