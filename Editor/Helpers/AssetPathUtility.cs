...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class AssetPathUtility
    {
        ...
        /// Private builds default to the project-local server under Tools/MCPForUnityServer.
        /// </summary>
        /// <returns>Local server source path, custom source, or null when no private server source is available.</returns>
        public static string GetMcpServerPackageSource()
        {
            return McpProjectSettings.GetServerPackageSource(() =>
            {
        ...
        /// Kept only for backwards compatibility with older call sites.
        /// </summary>
        ...
        public static bool TryGetPrivateServerSource(out string source, out string error)
        {
            source = GetMcpServerPackageSource();
            error = null;

            if (string.IsNullOrWhiteSpace(source))
            {
                error = "未找到本地 MCP Server 源码。请确认项目下存在 Tools/MCPForUnityServer，或在高级设置中指定本地 Server 目录。";
                return false;
            }

            if (!IsLocalPathLike(source))
            {
                error = $"私有化版本不允许使用远端 Server 源: {source}";
                return false;
            }

            string localPath = source.StartsWith("file://", StringComparison.OrdinalIgnoreCase)
                ? source.Substring(7)
                : source;

            if (!Directory.Exists(localPath) || !File.Exists(Path.Combine(localPath, "pyproject.toml")))
            {
                error = $"本地 MCP Server 目录无效或缺少 pyproject.toml: {localPath}";
                return false;
            }
        ...
        public static bool IsLocalServerPath()
        {
            ...
            return IsLocalPathLike(fromUrl);
        }

        /// <summary>
        /// Gets the local server path when the configured server source points to a local directory.
        ...
        private static bool IsLocalPathLike(string source)
        {
            if (string.IsNullOrWhiteSpace(source))
            {
                return false;
