    ...
    public static class CodexConfigHelper
    {
        public static string BuildCodexServerBlock(string uvPath)
        {
            ...
            if (useHttpTransport)
            {
                ...
                AddCodexHttpDefaults(unityMCP);
            }
            else
            {
                // Stdio mode: Use command and args
                var (uvxPath, fromUrl, packageName) = AssetPathUtility.GetUvxCommandParts();
                if (!AssetPathUtility.TryGetPrivateServerSource(out fromUrl, out var sourceError))
                {
        ...
        public static string UpsertCodexServerBlock(string existingToml, string uvPath)
        {
            ...
            {
                throw new InvalidOperationException("现有 Codex config.toml 无法解析，已停止自动写入以避免覆盖原配置。");
            }
        ...
        public static bool TryParseCodexServer(string toml, out string command, out string[] args, out string url)
        {
            ...
            if (!TryGetTable(servers, "unityMCP", out var unity)
                ...
                && !TryGetTable(servers, "UnityMCP_8080", out unity))
            {
                return false;
            }
        ...
        private static void AddCodexHttpDefaults(TomlTable unityMCP)
        {
            unityMCP["enabled"] = new TomlBoolean { Value = true };
            unityMCP["startup_timeout_sec"] = new TomlInteger { Value = 20 };
            unityMCP["tool_timeout_sec"] = new TomlInteger { Value = 180 };
        }
