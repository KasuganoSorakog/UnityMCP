    ...
    public class McpClientConfigSection
    {
        ...
        private string GetStatusDisplayString(McpStatus status)
        {
            return status switch
            {
                ...
                McpStatus.IncorrectPath => "路径不匹配",
                McpStatus.CommunicationError => "通信异常",
                McpStatus.NoResponse => "无响应",
                McpStatus.UnsupportedOS => "系统不支持",
        ...
        public void UpdateManualConfiguration()
        {
            ...
            {
                installationStepsLabel.text = "当前客户端没有可用的配置步骤。";
            }
        }
        ...
        private void UpdateClaudeCliPathVisibility()
        {
            ...
            if (client is ClaudeCliMcpConfigurator)
            {
                ...
                if (string.IsNullOrEmpty(claudePath))
                {
                    ...
                    claudeCliPath.value = "未找到，请点击“选择”手动指定";
                }
                else
                {
                    claudeCliPathRow.style.display = DisplayStyle.Flex;
        ...
        private void OnConfigureAllClientsClicked()
        {
            try
            {
                ...
                EditorUtility.DisplayDialog("配置所有客户端", message, "确定");
        ...
        private void OnBrowseClaudeClicked()
        {
            ...
            if (!string.IsNullOrEmpty(picked))
            {
                try
                {
                    ...
                    UpdateClaudeCliPathVisibility();
                    UpdateClientStatus();
                    McpLog.Info($"Claude CLI 路径已设置为: {picked}");
                }
                catch (Exception ex)
                {
        ...
        private void OnOpenFileClicked()
        {
            ...
            {
                McpLog.Error($"打开配置文件失败: {ex.Message}");
            }
        }
            ...
            if ((forceImmediate || needsRefresh) && !statusRefreshInFlight.Contains(client))
            {
                ...
                }).ContinueWith(t =>
                {
                    ...
                    if (t.IsFaulted && t.Exception != null)
                    {
                        ...
                        errorMessage = baseException?.Message ?? "状态检查失败";
                        McpLog.Error($"刷新 Claude CLI 状态失败: {errorMessage}");
                        faulted = true;
                    }
                    ...
                    EditorApplication.delayCall += () =>
                    {
                        ...
                        if (faulted)
                        {
                            ...
                            {
                                baseConfigurator.Client.SetStatus(McpStatus.Error, errorMessage ?? "状态检查失败");
                            }
                        }
                        ApplyStatusToUi(client);
                    };
