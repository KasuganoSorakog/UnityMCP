...
namespace MCPForUnity.Editor.Services
{
    ...
    public class PathResolverService : IPathResolverService
    {
        ...
        internal static string ResolveUvxFromSystem()
        {
            try
            {
        ...
        internal static IEnumerable<string> EnumerateUvxCandidates()
        {
            return EnumerateUvExecutableCandidates("uvx");
        }

        internal static IEnumerable<string> EnumerateUvCandidates()
        {
            return EnumerateUvExecutableCandidates("uv");
        }

        private static IEnumerable<string> EnumerateUvExecutableCandidates(string executableBaseName)
        {
            ...
            if (!string.IsNullOrEmpty(pathEnv))
            {
                foreach (string rawDir in pathEnv.Split(Path.PathSeparator))
                {
                    ...
                    if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                    {
                        ...
                        yield return Path.Combine(dir, executableBaseName);
                    }
                }
            }
        }
