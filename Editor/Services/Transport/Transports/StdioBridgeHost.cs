...
namespace MCPForUnity.Editor.Services.Transport.Transports
{
    ...
    public static class StdioBridgeHost
    {
        ...
        private static string cachedStatusDir;
        private static string cachedStatusFilePath;
        ...
        private static void InitializeAfterCompilation()
        {
            ...
            if (IsCompiling())
            {
                // 编译期间不继续排队重试，避免域重载时后台反复唤醒；需要用户在面板手动连接。
                return;
            }
        ...
        public static void Stop()
        {
            ...
            // 清空静态命令队列：取消所有 pending waiter（不再干等到 FrameIOTimeoutMs），
            // 避免残留条目在下次 Start 后污染新会话。在 startStopLock 块外执行，避免锁嵌套。
            lock (lockObj)
            {
                foreach (var kvp in commandQueue)
                {
                    try { kvp.Value.Tcs?.TrySetCanceled(); } catch { }
                }
                commandQueue.Clear();
            }

            // 取消已转交给命令分发器的 pending 命令（Runner 用的是独立 CTS，Stop 管不到），
            // 避免旧命令在 bridge 重启后继续执行
            try { TransportCommandDispatcher.CancelAllPending("stdio_stop"); } catch { }
                    ...
                    while (isRunning && !token.IsCancellationRequested)
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    // 超时：锁内条件移除——仅当命令仍排队（IsExecuting=false）时移除，
                                    // 与 ProcessCommands 快照标记（同一把 lockObj 下互斥）消除"移除后仍被执行"窗口；
                                    // 已在执行的命令由执行路径自己 Remove 并 Try* 完成 TCS，安全
                                    lock (lockObj)
                                    {
                                        if (commandQueue.TryGetValue(commandId, out var qc) && !qc.IsExecuting)
                                        {
        ...
        public static void WriteHeartbeat(bool reloading, string reason = null)
        {
            try
            {
                ...
                var payload = new
                {
                    ...
                    seq = heartbeatSeq,
                    project_path = cachedProjectPath,
                    project_name = cachedProjectName,
                    unity_version = cachedUnityVersion,
                    last_heartbeat = DateTime.UtcNow.ToString("O")
                };
                File.WriteAllText(cachedStatusFilePath, JsonConvert.SerializeObject(payload), new System.Text.UTF8Encoding(false));
            }
            catch (Exception)
            {
            }
        ...
        {
            EnsureStatusMetadataCache();
            return cachedStatusFilePath;
        }

        private static void EnsureStatusMetadataCache()
        {
            ...
            if (!string.IsNullOrEmpty(cachedStatusFilePath) &&
                string.Equals(cachedStatusDir, dir, StringComparison.OrdinalIgnoreCase) &&
                string.Equals(cachedProjectPath, projectPath, StringComparison.OrdinalIgnoreCase))
            {
                return;
            }

            // Heartbeat 是低频判活文件，缓存路径和项目信息，避免周期性重复算 hash / 创建目录。
            Directory.CreateDirectory(dir);
            cachedStatusDir = dir;
            cachedProjectPath = projectPath;
            cachedProjectName = ResolveProjectName(projectPath);
            cachedUnityVersion = Application.unityVersion;
            cachedStatusFilePath = Path.Combine(dir, $"unity-mcp-status-{ComputeProjectHash(projectPath)}.json");
        }

        private static string ResolveProjectName(string projectPath)
        {
            try
            {
