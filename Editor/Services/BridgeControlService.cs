    ...
    public class BridgeControlService : IBridgeControlService
    {
        ...
        public int CurrentPort
        {
            get
            {
                ...
                {
                    // HTTP 模式下端口来自中心 Server 的 base URL（默认 8080），不再回退到 legacy stdio 端口
                    if (Uri.TryCreate(HttpEndpointUtility.GetBaseUrl(), UriKind.Absolute, out var baseUri) && baseUri.Port > 0)
                    {
                        return baseUri.Port;
                    }
        ...
        public async Task<bool> StartAsync()
        {
            ...
            try
            {
                // 用户从面板主动连接时，解除域重载/断线后的手动连接保护，并清除本工程的被顶替标记。
                EditorPrefs.DeleteKey(EditorPrefKeys.ManualReconnectRequired);
                EditorPrefs.DeleteKey(EditorPrefKeys.SupersededPrefix + ProjectIdentityUtility.GetProjectHash());
