    ...
    public static class DependencyManager
    {
        ...
        private static void GenerateRecommendations(DependencyCheckResult result, IPlatformDetector detector)
        {
            ...
            foreach (var dep in missing)
            {
                if (dep.Name == "Python")
                {
                    result.RecommendedActions.Add($"安装 Python 3.10+：{detector.GetPythonInstallUrl()}");
                }
                else if (dep.Name == "uv Package Manager")
                {
                    result.RecommendedActions.Add($"安装 uv/uvx 包管理器：{detector.GetUvInstallUrl()}");
                }
                else if (dep.Name == "MCP Server")
                {
                    result.RecommendedActions.Add("MCP Server 会在需要时自动使用项目内的本地私有版本。");
                }
            }

            if (result.GetMissingRequired().Count > 0)
            {
                result.RecommendedActions.Add("请在 UnityMCP 主窗口点击“本地设置”按钮，按提示完成环境检查。");
            }
        }
    }
}
