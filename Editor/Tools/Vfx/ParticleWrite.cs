...
namespace MCPForUnity.Editor.Tools.Vfx
{
    internal static class ParticleWrite
    {
        public static object SetMain(JObject @params)
        {
            ...
            bool changesDuration = @params["duration"] != null;
            bool wasPlaying = changesDuration && ps.isPlaying;
            bool wasPaused = changesDuration && ps.isPaused;

            // duration 在粒子仍存活时不可修改，先清空预览，写入后再恢复原播放状态。
            if (changesDuration && (wasPlaying || wasPaused || ps.particleCount > 0))
            {
